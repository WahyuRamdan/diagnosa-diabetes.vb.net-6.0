...
Public Class Penyakit
    ...
    Private Function tampildata()
        Try
            connection.Open()
            ' Table ruangan
            Dim query1 As String = "SELECT * FROM daftar_penyakit"
    ...
    Private Sub DataGridView1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellClick
        If e.RowIndex >= 0 Then
            ...
            idHapus = selectedRow.Cells("id_penyakit").Value.ToString()
            indexGridView1 = selectedRow.Index
            Console.WriteLine(selectedRow.Index)
        End If
    End Sub
    Private Sub Penyakit_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tampildata()
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            connection.Open()
            Dim query As String = "UPDATE `daftar_penyakit` Set nama_penyakit = @nama, keterangan = @keterangan, np_penyakit = @nppenyakit WHERE `daftar_penyakit`.`id_penyakit` = @idHapus;"
            Dim command As New MySqlCommand(query, connection)
            ' Mengatur nilai parameter
            command.Parameters.AddWithValue("@idHapus", idHapus)
            ' Mengatur nilai parameter
            command.Parameters.AddWithValue("@nama", namaInput.Text())
            command.Parameters.AddWithValue("@keterangan", keteranganInput.Text())
            command.Parameters.AddWithValue("@nppenyakit", npInput.Text())
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        Try
            ...
            namaInput.Text = selectedRow.Cells("nama_penyakit").Value.ToString()
            keteranganInput.Text = selectedRow.Cells("keterangan").Value.ToString()
            npInput.Text = selectedRow.Cells("np_penyakit").Value.ToString()
        Catch ex As Exception
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        Try
            connection.Open()
            Dim query As String = "DELETE FROM daftar_penyakit WHERE `daftar_penyakit`.`id_penyakit` = @value1"
            Dim command As New MySqlCommand(query, connection)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            connection.Open()
            Dim query As String = "INSERT INTO daftar_penyakit (nama_penyakit, keterangan, np_penyakit) values (@nama, @keterangan, @np)"
            Dim command As New MySqlCommand(query, connection)
            command.Parameters.AddWithValue("@nama", namaInput.Text())
            command.Parameters.AddWithValue("@keterangan", keteranganInput.Text())
            command.Parameters.AddWithValue("@np", npInput.Text())
            Dim rowsAffected As Integer = command.ExecuteNonQuery()
            If rowsAffected > 0 Then
